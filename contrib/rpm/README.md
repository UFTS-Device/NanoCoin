...
The RPM spec file provided here is for NanoCoin-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://nanocoin.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[NanoCoin git repository](https://github.com/nanocoin/nanocoin) but are not part of
...
Currently only `Source10` is used, for the example `nanocoin.conf` file.
...
in the nanocoin git repository. At present only an SVG version of the NanoCoin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the nanocoin
...
needed when 0.12.1 is released, a proper fix is already in the NanoCoin git
master branch.
...
idea to build NanoCoin Core against a static version of that library compiled
...
The `nanocoin.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `nanocoin-server` package. At this point, most RPM based
...
The `nanocoin-server` package should have SELinux support. How to properly do
...
NanoCoin-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/nanocoin/). He has been
packaging NanoCoin for Fedora at least since 2012.
...
4. I renamed the `nanocoin` package that contains the Qt GUI to `nanocoin-core` as
that appears to be how the general population refers to it, in contrast to
`nanocoin-xt` or `nanocoin-classic`. I wanted to make sure the general population
