...
###   This script attempts to download the signature file SHA256SUMS.asc from nanocoin.org
...
WORKINGDIR="/tmp/nanocoin"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test/"
BASEDIR="https://nanocoin.org/bin/"
VERSIONPREFIX="nanocoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'nanocoin-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the NanoCoin Core binary release signing key installed?"
   fi
